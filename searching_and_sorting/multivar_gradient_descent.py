...
def gradient_descent(f, f_deriv, initial, learning_rate):
    ...
    results = [["n", "x", "gradient at x", "f(x)"]]
    gradient = f_deriv(initial)
    n = 1
    x = initial
    results.append([n, initial, gradient, f(x) if f != None else None])
    while gradient_not_flat(gradient, 9):
        n += 1
        delta_x = scalar_multiply(gradient, learning_rate)
        x = minus(x, delta_x)
        gradient = f_deriv(x)
        results.append([n, x, gradient, f(x) if f != None else None])
        if (n == max_iter):
            break
...
if __name__ == "__main__":
    ...
    for x in range(-1, 2):
        for y in range(-1, 2):
            for z in range(-1, 2):
                try:
                    results = gradient_descent(f_3, f_3_grad, [x, y, z], 0.01)
